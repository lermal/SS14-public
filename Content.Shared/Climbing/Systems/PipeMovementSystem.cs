...
public sealed partial class PipeMovementSystem : VirtualController
{
    ...
    private const string PipeMovementFixtureName = "pipeMovement";
    private const int PipeMovementCollisionGroup = (int) (CollisionGroup.TableLayer | CollisionGroup.LowImpassable);

    private EntityQuery<PipeMovementComponent> _pipeMovementQuery;
    ...
    public override void Initialize()
    {
        ...
        _pipeMovementQuery = GetEntityQuery<PipeMovementComponent>();
        _fixturesQuery = GetEntityQuery<FixturesComponent>();
        _xformQuery = GetEntityQuery<TransformComponent>();

        SubscribeLocalEvent<PipeMovementComponent, UpdateCanMoveEvent>(OnMoveAttempt);
        SubscribeLocalEvent<PipeMovementComponent, ClimbDoAfterEvent>(OnDoAfter);
        SubscribeLocalEvent<PipeMovementComponent, EndCollideEvent>(OnClimbEndCollide);
    }
    ...
    private void OnMoveAttempt(EntityUid uid, PipeMovementComponent component, UpdateCanMoveEvent args)
    {
        // Get the transform of the entity
        var xform = _xformQuery.GetComponent(uid);
        var currentPos = _xformSystem.GetWorldPosition(xform);

        // Get movement direction from args
        var moveDir = args.Direction;
        if (moveDir == Vector2.Zero)
            return;

        // Check if there is a pipe in the movement direction
        var checkPos = currentPos + moveDir;
        var hasPipe = false;

        // Query for nearby pipes
        var query = GetEntityQuery<PipeNode>();
        var xformQuery = GetEntityQuery<TransformComponent>();

        foreach (var pipe in query)
        {
            if (!xformQuery.TryGetComponent(pipe.Owner, out var pipeXform))
                continue;

            var pipePos = _xformSystem.GetWorldPosition(pipeXform);

            // If pipe is close enough in the movement direction
            if ((pipePos - checkPos).Length() < 0.5f)
            {
                hasPipe = true;
                break;
            }
